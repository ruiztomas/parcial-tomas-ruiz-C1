...
const reglas=require("./politica/reglas.js");
...
const mapperConReglas=(u)=>{
    return {
        ...mapperSimple(u),
        reglasIncumplidas: reglasNoCumple(u),
    };
};

app.get("/validador", (req,res)=>{
    const respuesta=filtro(usuarios).map((u)=>mapperSimple(u));
    res.status(200).json(respuesta);
});

app.get("/validador/con-reglas", (req,res)=>{
    const respuesta=filtro(usuarios).map((u)=>mapperConReglas(u));
    res.status(200).json(respuesta);
});
app.get("/usuarios-correctos", (req,res)=>{
    const respuesta=usuarios
        .filter((u)=>!filtro(usuarios).includes(u))
